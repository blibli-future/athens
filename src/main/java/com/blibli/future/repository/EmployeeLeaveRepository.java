...
public interface EmployeeLeaveRepository extends JpaRepository<EmployeeLeave, Long>{
	...
	@Query("select new com.blibli.future.vo.ApprovalResponseVo(el.id, el.employee.nik, e.fullName, el.startDate, el.endDate) "
			+ "from EmployeeLeave el join Employee e on el.employee.nik = e.nik where e.chiefNik = (?1) and e.status = (?2)")
