...
public interface EmployeeAbsencePermitRepository extends JpaRepository<EmployeeAbsencePermit, String>{
	...
	@Query(value = "select new com.blibli.future.vo.SickEmployeeVo(e.nik, e.fullName, e.nameOfDept, count(e)) from EmployeeAbsencePermit eap JOIN Employee e ON eap.employee.nik = e.nik WHERE e.nameOfDept = (?1) and eap.absencePermit = (?4) and (eap.requestDate between (?2) and (?3)) group by e.nik")
	public List<SickEmployeeVo> countSickEmployee(String deparment, LocalDate startDate, LocalDate endDate, AbsencePermit absencePermit);
}
